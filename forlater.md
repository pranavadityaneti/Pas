...
- **Customers — follow-ups (low priority):** (1) `/orders` page needs to respect `?userId=X` query param so the "View orders" deep link filters. Right now it navigates but shows all orders. (2) `/customer-support` page needs to respect `?phone=X` so the "Wati history" link filters the inbox to that customer's thread. Both are presentational — backends already have the data.
