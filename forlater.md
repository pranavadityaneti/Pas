...
#### ✅ PHASE 2 FINAL (storeId/branch_id rework) — Option B chosen (eliminate the column, not patch the FK). 2026-06-16.
  ...
  - **F4 (MEDIUM, structural):** make `merchant_branches.store_id` NOT NULL so the DB itself forbids a store-less branch. **(a) ✅ DONE — F4a deleted the 21 orphan/test branches** (`scripts/phase2final_F4a_delete_orphan_branches.ts`; re-verified 0 inbound refs in-tx; 5 branches remain, all with store_id). **(b) ⛔ PENDING explicit go — F4b `ALTER ... store_id SET NOT NULL`** (`scripts/phase2final_F4b_store_id_not_null.ts`, written, blocked by the migration classifier). Now SAFE to apply (F1 is deployed). Until applied, the chain is self-sustaining via F1 code (every creation path sets store_id) but not DB-enforced. F4b also runs a negative test (NULL insert rejected) + final 0-leak chain check.
  ...
  - **⛔ GATED — B10 DROP `StoreProduct.storeId` COLUMN — DO NOT APPLY until the 2 client surfaces propagate.** Shipped admin-web + merchant-app bundles read the column DIRECTLY via Supabase; dropping it now breaks them. **Rollout order:** (1) ✅ API deployed; (2) push branch → Vercel so admin-web `StoreProductTable` (branch_id) goes live; (3) ship a **merchant OTA** (per the .env protocol) carrying `AddMenuProductModal` + `useInventory` branch_id fixes; (4) after the merchant OTA has propagated, run `ALTER TABLE "StoreProduct" DROP COLUMN "storeId";` + drop the now-orphan `store` relation from the Prisma model + the `Store.products` inverse. Verify Freshly storefront returns its full catalog. F2 already removed the only DB-function dependency.
...
### EMPTY-STORE behavior (verified 2026-06-15, Pranav-flagged) — CONFIRMED
- A store with no active inventory STILL appears in the nearby/discovery list (`get_nearby_stores` is location-only, doesn't check inventory), but its storefront shows nothing (`useProducts` filters `active=true`; no graceful empty-state). Directly relevant to the "load Blinkit INACTIVE" guard — new merchants' stores would look empty until they price+activate. **Decision needed: hide zero-active-product stores from discovery, or show a clear empty state.**
