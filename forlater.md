...
- ⚠️ **[NEEDS VERIFICATION — possible money/trust bug] Legacy `POST /orders/:id/refund` SIMULATES refunds.** The merchant app (`useOrders.ts:482`) calls this endpoint to refund orders, but the real Razorpay refund call is **commented out** (`index.ts:~4234-4240`) — it fabricates a `rfnd_test_<ts>` id, flips the order to REFUNDED, and sends a "Refund Processed" SMS, but **no money actually moves.** VERIFY whether merchants use this for real customer refunds (vs. real refunds going only through admin `/admin/orders/:id/refund` + the WS2 cancel/return paths). If merchants rely on it → customers are told they're refunded but aren't. Fix = wire the real `razorpayInstance.payments.refund(paymentId, …)` against the order's `metadata.razorpayPaymentId`, then also fire `REFUND_INITIATED` there. Scope: `apps/api/src/index.ts` legacy refund endpoint + `apps/merchant-app/src/hooks/useOrders.ts`. Status: flagged, NOT fixed (out of scope). Date 2026-06-25. Originated: reading refund paths during #14 Phase 2.
