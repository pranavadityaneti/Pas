...
- **#14** Full notification scenario coverage (customer 14-scenario plan across 4 phases + merchant gaps).
  - ✅ **Phase 1 SHIPPED (2026-06-25):** recipient_role read cutover (both inboxes) + manual low-stock alert moved server-side + 12-row NULL→merchant backfill. EB `app-260625_105637612928` + consumer OTA `2951458e` (rt 1.1.3, incl. held H-5) + merchant OTA `f8eed436` (rt 1.2.6). Report `docs/notifications-phase1-execution-2026-06-25.html`. Audit: exactly 2 notif-write paths, both set recipient_role → leak class fully closed.
  - ✅ **Phase 2 SHIPPED (2026-06-25, API-only EB `app-260625_143131848924`):** 3 new consumer lifecycle notifications — `ORDER_REQUEST_EXPIRED` (expiry cron, per-row CAS idempotency), `PAYMENT_FAILED` (razorpay `payment.failed` webhook, idempotent on payment id), `REFUND_INITIATED` (admin refund only — cancel/return/SLA paths already notify). No OTA needed (cutover live → new types show by role).
  - ⏳ **DEFERRED — `REFUND_CREDITED` (D5):** needs Pranav to subscribe to the Razorpay **`refund.processed`** event in the Razorpay Dashboard, THEN a handler branch in `POST /webhooks/razorpay` → resolve consumer from refund→payment→order notes → notify "refund credited." Blocked on the dashboard subscription (external). Originated: #14 Phase 2.
  - ⏳ **DEFERRED — merchant business notifications** (KYC decision, payout/settlement) — a later "#14 merchant phase" once settlement leaves CRON_DRY_RUN + KYC UX (#11) lands.
  - 🎨 **Minor polish:** add dedicated icons for the 3 new consumer types in `NotificationToast.tsx` TYPE_CONFIG (they currently fall back to DEFAULT) — ride the next consumer OTA.
- 🔧 **Align merchant app to the admin-only-refunds model.** ✅ **Phase 1 SHIPPED (2026-06-25):** order-reject refund stripped + `refundOrder` removed (commit `c717c30a`, merchant OTA `2064f616`) → a rejected *paid* order stays CANCELLED+paid and surfaces in the admin Refunds & Disputes queue for the admin to refund. 📋 **Phase 3 (returns/exchanges → admin)** — spec `docs/refunds-admin-only-phase3-returns-spec-2026-06-25.html`. Decisions APPROVED (Pranav 2026-06-26): D1 keep SLA auto-approve cron, D2 move exchanges too, D3 keep read-only merchant inbox. **✅ 3a SHIPPED** (commit `a8174137`, EB live 2026-06-26): `decideOrderIssue()` shared engine + `GET /admin/issues` + `PATCH /admin/orders/:id/issue/:issueId` (requireAdmin, audited); merchant route behavior-preserved. **✅ 3b SHIPPED to PREVIEW** (commit `e825d7cb`, pushed feat→Vercel preview): admin-web `ReturnsExchanges.tsx` queue (list/approve/reject/refund) + router `/returns` + sidebar (gated `disputes.standard.resolve`). **✅ 3c verified** (Pranav OK'd Pending/Resolved). **✅ feat→main MERGED** (PR #13 → main `024b195a`, Vercel prod build SUCCESS) → admin.pickatstore.io now has the Returns & Exchanges tab. Backup tag `backup/main-pre-merge-2026-06-26`. **✅ 3d SHIPPED** (commit `ef5c6750`, EB + merchant OTA `d17f2111`): merchant decision route → 403; merchant Returns inbox → display-only (Pending = "under review by PAS team", Resolved = outcome; approve/reject + handleDecision removed). **✅ 3e** = SLA cron kept (D1). **RETURNS→ADMIN MIGRATION COMPLETE.**
  - ⚠️ **AUDIT 2026-06-26 — "merchants never refund" is NOT yet enforced server-side (Phase 1 was a CLIENT-ONLY patch). Two open refund paths:**
    - ✅ **[A1 DONE 2026-06-26, EB `app-260626_095745712534`, commit `55198099`] `POST /orders/:id/refund` → 403 tombstone.** Legacy merchant-gated simulated-refund route retired; stale builds can't fake a REFUNDED state. Real refunds stay admin-only.
    - ✅ **[A2 DONE 2026-06-26, commit `55198099`] Booking-cancel secured.** Added `userManagesBranch` ownership check (closes the IDOR on `PATCH /bookings/:bookingId/status` + `GET /bookings/merchant`) + status compare-and-set on the transition (kills the double-refund). Legit merchants still pass. ⏳ **Remaining:** (a) the booking-fee refund is still ISSUED by the merchant cancel (now safe/owner-only/once) — moving it strictly to admin ("merchants never refund") needs an admin booking-refund path; DECIDE admin-manual vs keep-the-safe-instant-auto-refund for the small ₹25 fee. (b) the OTHER bookings-router endpoints (`PATCH /service-modes/:branchId` + the handlers at `getAuthUser` lines ~42/76/140/308) STILL lack ownership checks → apply `userManagesBranch` to them too (ownership sweep).
    - paid-order-cancel notification copy ("a refund will be processed by our team").
  - 🛠 **Notifications hardening (audit 2026-06-26, all Med/Low — class is closed, these are durability/infra):** (a) ORDER_REQUEST_EXPIRED notify is fire-and-forget AFTER the status flips to EXPIRED → a failed dispatch silently drops the "you weren't charged" message, non-retryable (`scheduled-jobs.ts` expireStaleOrderRequests); make notify atomic/retryable. (b) manual low-stock crossing reads pre-edit stock via Prisma but writes via supabaseAdmin + compares the requested value → racy crossing under concurrent decrement (alerting-only). (c) PAYMENT_FAILED/REFUND_INITIATED dedupe is check-then-act, not constraint-enforced (dup-toast only). (d) **commit `ALTER PUBLICATION supabase_realtime ADD TABLE notifications;` as a migration** — realtime guards depend on publication membership that exists only in a code comment (would break on env rebuild). (e) returns.tsx stale header + dead approve/reject styles cleanup.
  - 🔧 **Git:** feat is ahead of main by a few commits (checklist doc + 3d). 3d shipped via EB+OTA (not main), so prod is correct; reconcile feat→main at the next admin-web change. Test seed for the Returns demo (order `bb6b2757`, user `84cc8540`) can be deleted when done — command in the chat. ⏳ **Phase 2 (booking-cancel) + paid-cancel copy + delete dead simulated `POST /orders/:id/refund`** still pending. DECISION (Pranav, restated 2026-06-25): **merchants NEVER refund** — refunds are executed only from the admin dashboard (`POST /admin/orders/:id/refund`, real Razorpay); the merchant app is **display-only** (just shows what was refunded). Today the merchant app still *initiates* refunds in two flows that contradict this: (1) **order-reject of a paid order** → `refundOrder` (`apps/merchant-app/src/hooks/useOrders.ts:482` ← `app/(main)/orders.tsx:161`) → legacy `POST /orders/:id/refund`, which SIMULATES (real Razorpay call commented out, `index.ts:~4234`) → flips order to REFUNDED + sends a "Refund Processed" SMS with **no real money moved**; (2) **return/exchange approval** (`app/(main)/settings/returns.tsx`) → WS2 issue endpoint → **real** Razorpay refund. Desired end state: strip merchant refund-initiation, merchants display-only, all real refunds via admin. **Open design Qs for Pranav:** when a merchant rejects a paid order or approves a return, what state should the order enter, and who triggers the actual refund — admin-manual, or server-auto (refund happens as a server consequence of the merchant's reject/approve, not a "merchant-initiated" button)? Also: the premature "Refunded" status + SMS on reject should wait until a real refund occurs. NOT a money bug (merchants aren't meant to refund) — a flow-alignment task. Scope: `apps/merchant-app` (useOrders/orders/returns) + `apps/api` refund endpoints. Status: flagged, NOT fixed. Date 2026-06-25. Originated: #14 Phase 2 refund review + Pranav clarification. See memory `refunds-admin-only`.
