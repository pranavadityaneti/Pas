...
- **REMAINING 9a (next session):** refactor apps to call these — merchant-app store-details.tsx → PATCH /merchant/profile; payouts.tsx (handleAddBank + handleSetPrimaryBank + any other bank writes) → PUT /merchant/payout; admin-web useMerchants.createMerchant/updateMerchant → POST/PATCH /admin/merchants; MerchantDirectory.tsx status → PATCH /admin/merchants/:id. Then exhaustive re-grep (0 direct merchants/Store writes) → GATED lockdown (REVOKE writes on merchants + Store, drop qual=true policies incl. "Enable all operations for anon", keep SELECT). Gate on merchant OTA.
