...
- **🔥 ROOT CAUSE FIX 2026-06-03 night — wrong-table bug class** (founder audit caught it). Two order tables existed in Supabase: lowercase `orders` (demo seed from apps/api/create_orders.sql with NO user_id / branch_id / total_amount) AND production `"Order"` (capital O via Prisma, the real one). `useCustomers.ts` AND `useOrders.ts` were both `.from('orders')` (lowercase) which resolved to the demo table. Result: Customers page showed 0 orders / ₹0 LTV / Unknown location for every customer (even though founder placed 15-16 orders + team placed 50-100). `/orders` page was showing stale demo data or nothing. Fixed BOTH hooks: useCustomers uses `orders:Order!fk_orders_user(...)` embed with explicit FK hint; useOrders uses `.from('Order')` directly. Also rebuilt OrderManager UI around the real 6-state enum (PENDING/CONFIRMED/READY/COMPLETED/CANCELLED/REFUNDED) — removed the lowercase demo enum, the fake SLA column, the fake driver/delivery block ("Looking for drivers… Zone: Indiranagar"), the hardcoded timeline (10:45 AM/10:47 AM), and the fake customer/merchant sheet reconstruction. NOT touched: the consumer-side POST /orders flow including the 4-layer FK-race hardening from 2026-05-29.
