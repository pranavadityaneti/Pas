...
async function handlePaymentCaptured(payment) {
    ...
    if (existing) {
        console.log('[razorpay-webhook] Subscription already exists for paymentId=', paymentId, ' — idempotent no-op');
        return;
    }
    ...
    if (!merchantId || paymentType !== 'merchant_signup') {
        ...
        // settlement path.
        console.log('[razorpay-webhook] No merchant_signup notes on order', orderId, '— skipping reconciliation. notes=', JSON.stringify(notes));
        return;
    }
    // Confirm the merchant exists. If not, we can't safely insert.
    const merchant = await prisma.merchant.findUnique({ where: { id: merchantId } });
    if (!merchant) {
        console.error('[razorpay-webhook] payment.captured but merchant', merchantId, 'does not exist in DB. Payment', paymentId, 'requires manual reconciliation.');
        return;
    }
    ...
    console.log('[razorpay-webhook] Reconciled missing Subscription for merchant=', merchantId, 'paymentId=', paymentId, 'amount=', amountInr);
...
app.get('/products', async (req, res) => {
    try {
        ...
        if (minPrice || maxPrice) {
            where.mrp = {};
            if (minPrice)
                where.mrp.gte = Number(minPrice);
            if (maxPrice)
                where.mrp.lte = Number(maxPrice);
        }
        if (missingData) {
        ...
        return (0, api_error_handler_1.handleApiError)(res, error, { area: 'products.list', userMessage: 'Failed to load products' });
    }
});
...
async function processScraperDataset(datasetId) {
    ...
    const sanitizeScraperData = (obj) => {
        ...
        if (Array.isArray(obj))
            return obj.map(sanitizeScraperData);
...
app.post('/catalog/sync/trigger', async (req, res) => {
    ...
    try {
        ...
        const run = await apify_service_1.apifyService.triggerLiveSync(expandedQueries, location, finalLimit);
...
async function logBulkAudit(items, action, changedBy = 'system', tx) {
    if (items.length === 0)
        return;
    ...
    return client.productAuditLog.createMany({ data: logs });
}
// --- TAXONOMY VALIDATION HELPER ---
async function validateTaxonomy(verticalId, category_id) {
    const category = await prisma.tier2Category.findUnique({
...
app.delete('/categories/:id', async (req, res) => {
    ...
    try {
        await prisma.$transaction(async (tx) => {
            ...
            await tx.tier2Category.delete({ where: { id } });
        });
...
app.post('/catalog/sync/approve', async (req, res) => {
    ...
    try {
        ...
        await prisma.$transaction(async (tx) => {
            ...
            const idBySource = new Map(auditProducts.map((p) => [String(p.sourceProductId), p.id]));
            await logBulkAudit(resolvedItems
                .map((item) => ({ id: idBySource.get(String(item.source_product_id || item.sourceProductId)), item }))
                .filter((x) => x.id), 'CATALOG_SYNC_APPROVE', 'system', tx);
            // 6. Batch Cleanup
...
app.post('/products/bulk-update', async (req, res) => {
    ...
    try {
        ...
        const updateData = {};
        if (updates.vertical_id !== undefined)
            updateData.vertical_id = updates.vertical_id;
        ...
        if (updates.category !== undefined) {
            ...
            if (t2?.verticalId && updateData.vertical_id === undefined)
                updateData.vertical_id = t2.verticalId;
        }
        if (updates.gstRate !== undefined)
            updateData.gstRate = parseFloat(updates.gstRate);
        if (updates.unitType !== undefined)
            updateData.unitType = updates.unitType;
        ...
        if (updateData.vertical_id || updateData.category_id) {
            if (updateData.vertical_id && updateData.category_id) {
                ...
                if (!isValid)
                    return res.status(400).json({ error: 'Invalid Category/Vertical pairing' });
            }
        }
...
app.patch('/products/:id', async (req, res) => {
    ...
    if (updates.mrp !== undefined)
        updateData.mrp = parseFloat(updates.mrp);
    ...
    if (updates.ean !== undefined)
        updateData.ean = updates.ean;
    if (updates.image !== undefined)
        updateData.image = updates.image;
    if (updates.unitType !== undefined)
        updateData.unitType = updates.unitType;
    if (updates.unitValue !== undefined)
        updateData.unitValue = updates.unitValue;
    if (updates.hsnCode !== undefined)
        updateData.hsnCode = updates.hsnCode;
    if (updates.gstRate !== undefined)
        updateData.gstRate = updates.gstRate;
    try {
        ...
        if (updateData.vertical_id && updateData.category_id) {
            const isValid = await validateTaxonomy(updateData.vertical_id, updateData.category_id);
            if (!isValid)
                return res.status(400).json({ error: 'Invalid Category for the selected Vertical' });
        }
        ...
        return (0, api_error_handler_1.handleApiError)(res, error, { area: 'products.patch', extra: { id }, userMessage: 'Failed to update product' });
    }
});
...
app.post('/products/:id/images', async (req, res) => {
    ...
    try {
        ...
        return (0, api_error_handler_1.handleApiError)(res, error, { area: 'products.add-image', extra: { id }, userMessage: 'Failed to add image' });
    }
});
// Delete Image
            ...
            for (let i = 0; i < batch.length; i++) {
                ...
                try {
                    ...
                    if (raw.category_hierarchy) {
                        const parts = String(raw.category_hierarchy).split(/[>\/|,]/).map((s) => s.trim()).filter(Boolean);
                    ...
                    mapped.mrp = mrp;
                    if (mapped.avgRating)
                        mapped.avgRating = parseFloat(mapped.avgRating) || null;
                    if (mapped.numberOfRatings)
                        mapped.numberOfRatings = parseInt(mapped.numberOfRatings) || null;
                    if (mapped.unitPrice)
                        mapped.unitPrice = parseFloat(mapped.unitPrice) || null;
                    if (mapped.shippingCharges)
                        mapped.shippingCharges = parseFloat(mapped.shippingCharges) || null;
                    if (mapped.isSoldOut)
                        mapped.isSoldOut = mapped.isSoldOut === true || mapped.isSoldOut === 'true' || mapped.isSoldOut === 1;
...
app.get('/consumer/stores', async (req, res) => {
    try {
        ...
        const formatted = stores.map(store => ({
            ...
            product_count: store._count?.products || 0,
        }));
        res.json({
            data: formatted,
...
app.get('/consumer/stores/:id', async (req, res) => {
    try {
        ...
        const productWhere = { storeId: id, active: true };
        if (category) {
...
app.post('/orders', async (req, res) => {
    ...
    try {
        ...
        const order = await prisma.$transaction(async (tx) => {
            ...
            const stockItems = items.filter((item) => item.storeProductId);
...
app.patch('/orders/:id/status', async (req, res) => {
    ...
    try {
        ...
        const ordStoreId = order.store?.id || order.storeId;
        if (status === 'CANCELLED') {
        ...
        if (order.user?.phone) {
            ...
            if (smsStatus) {
                sms_service_1.smsService.sendOrderUpdate(order.user.phone, id, smsStatus).catch(err => console.error('SMS Failed:', err));
            }
        }
 ...
 */
async function userCanManageOrderStore(userId, storeId) {
...
app.post('/push-tokens/register', async (req, res) => {
    try {
        ...
        const token = await prisma.merchantPushToken.upsert({
            where: {
...
app.delete('/push-tokens/deregister', async (req, res) => {
    try {
        ...
        await prisma.merchantPushToken.updateMany({
...
app.get('/debug/list-merchants', async (req, res) => {
    try {
        const merchants = await prisma.$queryRawUnsafe(`SELECT * FROM merchants`);
...
app.post('/auth/send-otp', async (req, res) => {
    ...
    try {
        ...
        let sent = false;
        try {
            sent = await wati_service_1.watiService.sendOtp(phone, otp);
        }
        catch (watiErr) {
    ...
    try {
        ...
        await prisma.$transaction(async (tx) => {
            ...
            if (payload.storeName !== undefined)
                updateData.storeName = payload.storeName;
            if (payload.verticalId !== undefined)
                updateData.verticalId = payload.verticalId;
            ...
            if (payload.hasBranches !== undefined)
                updateData.hasBranches = payload.hasBranches;
            ...
            if (payload.aadharNumber !== undefined)
                updateData.aadharNumber = payload.aadharNumber;
            if (payload.msmeNumber !== undefined)
                updateData.msmeNumber = payload.msmeNumber;
            if (payload.bankAccount !== undefined)
                updateData.bankAccountNumber = payload.bankAccount;
            if (payload.ifsc !== undefined)
                updateData.ifscCode = payload.ifsc;
            if (payload.beneficiaryName !== undefined)
                updateData.bankBeneficiaryName = payload.beneficiaryName;
            if (payload.turnoverRange !== undefined)
                updateData.turnoverRange = payload.turnoverRange;
            if (payload.gstNumber !== undefined)
                updateData.gstNumber = payload.gstNumber;
            if (payload.fssaiNumber !== undefined)
                updateData.fssaiNumber = payload.fssaiNumber;
            if (payload.docUrls) {
                if (payload.docUrls.pan)
                    updateData.panDocUrl = payload.docUrls.pan;
                if (payload.docUrls.aadharFront)
                    updateData.aadharFrontUrl = payload.docUrls.aadharFront;
                if (payload.docUrls.aadharBack)
                    updateData.aadharBackUrl = payload.docUrls.aadharBack;
                if (payload.docUrls.msme)
                    updateData.msmeCertificateUrl = payload.docUrls.msme;
                if (payload.docUrls.gst)
                    updateData.gstCertificateUrl = payload.docUrls.gst;
                if (payload.docUrls.fssai)
                    updateData.fssaiCertificateUrl = payload.docUrls.fssai;
            }
            ...
            if (Array.isArray(payload.stores) && payload.stores.length > 0 && payload.stores[0]?.city) {
                ...
                const submittedIds = payload.stores.map((s) => s.id);
            ...
            if (payload.subscription) {
                ...
                if (payload.couponCode && Number(payload.couponDiscount) > 0) {
                    ...
                    if (Number(payload.couponDiscount) !== coupon.discountInr) {
                        ...
                        throw new Error(`Coupon ${normalized} discount mismatch (expected ₹${coupon.discountInr}).`);
                    }
        ...
        if (payload.finalize) {
            ...
            if (targetEmail) {
                (0, email_service_1.sendApplicationReceivedEmail)(targetEmail, targetName)
...
app.post('/merchants/:id/kyc-decision', async (req, res) => {
    ...
    try {
        ...
        if (decision === 'approve') {
            ...
            if (merchant.email) {
                (0, email_service_1.sendStoreApprovedEmail)(merchant.email, merchant.ownerName || 'Partner', merchant.storeName || 'Your Store')
            ...
            if (merchant.email) {
                (0, email_service_1.sendStoreRejectedEmail)(merchant.email, merchant.ownerName || 'Partner', rejectionReason || '')
 ...
 */
const merchantSignupSchema = zod_1.z.object({
    ...
    panNumber: zod_1.z.string().min(10, "Valid PAN is required"),
    aadharNumber: zod_1.z.string().min(12, "Valid Aadhaar is required"),
    msmeNumber: zod_1.z.string().optional().nullable(),
    bankAccount: zod_1.z.string().min(9, "Valid bank account is required"),
    ifsc: zod_1.z.string().min(4, "Valid IFSC is required"),
    beneficiaryName: zod_1.z.string().min(2, "Beneficiary name is required"),
    turnoverRange: zod_1.z.string(),
    gstNumber: zod_1.z.string().min(15, "Valid GST is required"),
    fssaiNumber: zod_1.z.string().optional().nullable(),
    docUrls: zod_1.z.object({
        pan: zod_1.z.string().min(1, "Valid PAN doc required"),
        aadharFront: zod_1.z.string().min(1, "Valid Aadhaar front required"),
        aadharBack: zod_1.z.string().min(1, "Valid Aadhaar back required"),
        msme: zod_1.z.string().nullable().optional(),
        gst: zod_1.z.string().min(1, "Valid GST doc required"),
        fssai: zod_1.z.string().nullable().optional(),
    }),
    storePhotos: zod_1.z.array(zod_1.z.string()),
    branches: zod_1.z.array(zod_1.z.object({
        name: zod_1.z.string().optional(),
        address: zod_1.z.string().optional(),
        manager_name: zod_1.z.string().optional(),
...
app.get('/admin/config', async (req, res) => {
    ...
    if (!caller)
        return;
    try {
        const rows = await prisma.$queryRawUnsafe(`SELECT key, value FROM public.platform_settings`);
        const settings = {};
        for (const r of rows)
...
app.get('/config/public', async (_req, res) => {
    try {
        const rows = await prisma.$queryRawUnsafe(`SELECT key, value FROM public.platform_settings WHERE key IN ('service_radius_km','min_order_value')`);
        const cfg = { serviceRadiusKm: 10, minOrderValue: 0 };
        for (const r of rows) {
            if (r.key === 'service_radius_km')
                cfg.serviceRadiusKm = Number(r.value);
            if (r.key === 'min_order_value')
                cfg.minOrderValue = Number(r.value);
        }
        return res.json(cfg);
    }
    catch {
...
app.get('/admin/wati/threads', async (req, res) => {
    ...
    try {
        ...
        for (const r of rows) {
            ...
            t.count++;
            if (r.direction === 'inbound' && !r.isRead)
                t.unread++;
        }
        return res.json({ threads: Array.from(byPhone.values()) });
    }
    catch (e) {
        return (0, api_error_handler_1.handleApiError)(res, e, { area: 'wati.threads', extra: undefined, userMessage: 'Failed to load conversations' });
    }
});
...
app.get('/admin/wati/thread', async (req, res) => {
    ...
    try {
        ...
        return (0, api_error_handler_1.handleApiError)(res, e, { area: 'wati.thread', extra: undefined, userMessage: 'Failed to load conversation' });
    }
});
...
app.post('/admin/wati/reply', async (req, res) => {
    ...
    try {
        ...
        const result = await wati_service_1.watiService.sendSessionMessage(phone, body);
        if (!result.ok) {
        ...
        const saved = await prisma.watiInbox.create({
            data: {
                ...
                messageType: 'text',
                body,
                rawPayload: { sentBy: caller.id, via: 'admin-support-inbox' },
                status: 'open',
                isRead: true,
            },
        });
        ...
        return (0, api_error_handler_1.handleApiError)(res, e, { area: 'wati.reply', extra: undefined, userMessage: 'Failed to send reply' });
    }
});
/**
...
app.get('/admin/disputes', async (req, res) => {
    try {
        ...
        const shape = (o) => ({
            ...
            hasPaymentId: !!(o.metadata?.razorpayPaymentId),
        ...
        const notYetRefunded = pendingRefunds.filter((o) => !(o.metadata?.razorpayRefundId));
...
app.post('/admin/orders/:id/refund', async (req, res) => {
    try {
        ...
        if (!order)
            return res.status(404).json({ error: 'Order not found' });
        if (!order.isPaid)
            return res.status(400).json({ error: 'Order is not paid — nothing to refund' });
        if (order.status === 'REFUNDED' || order.metadata?.razorpayRefundId) {
            return res.status(409).json({ error: 'Order has already been refunded' });
        }
        const amountInr = Math.round(order.totalAmount || 0);
        if (amountInr <= 0)
            return res.status(400).json({ error: 'Order amount is zero' });
    ...
    try {
        ...
        if (preEdit && stockIsChanging) {
            ...
            try {
                ...
                if (after && after.stock != null)
                    committedStock = Number(after.stock);
            }
            catch (e) {
                console.error('[PATCH /merchant/store-products] post-edit stock re-read failed:', e);
            }
