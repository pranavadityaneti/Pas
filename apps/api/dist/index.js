...
async function createNotification(userId, storeId, type, title, message, link) {
    try {
    ...
    try {
        ...
        const order = await prisma.order.create({
            data: {
                ...
                status: 'PENDING',
                isPaid: false,
                branchId: storeId, // Added to fix missing property error
                items: {
        ...
        if (store && store.managerId) {
            ...
            await createNotification(store.managerId, storeId, 'ORDER', 'New Order Received!', `Order #${orderNumber} for ₹${totalAmount}`, `/orders/${order.id}`);
            ...
            for (const item of items) {
                try {
                    ...
                    if (sp.stock === 0) {
                        await createNotification(store.managerId, storeId, 'INVENTORY', 'Out of Stock Alert', `${sp.product.name} is completely out of stock.`, `/inventory`);
                    }
                    else if (sp.stock <= 5) {
                        await createNotification(store.managerId, storeId, 'INVENTORY', 'Low Stock Warning', `Action Required: Only ${sp.stock} left of ${sp.product.name}.`, `/inventory`);
                    }
                }
                catch (err) {
...
app.patch('/orders/:id/status', async (req, res) => {
    try {
        ...
        if (order.store?.managerId) {
            ...
            if (status === 'CANCELLED') {
                await createNotification(order.store.managerId, ordStoreId, 'ORDER', 'Order Cancelled', `Order #${order.orderNumber} was cancelled by the customer.`, `/orders/${id}`);
            }
            else if (status === 'RIDER_ARRIVED') {
                await createNotification(order.store.managerId, ordStoreId, 'DELIVERY', 'Rider Waiting', `Rider is at the store to pickup Order #${order.orderNumber}.`, `/orders/${id}`);
            }
            else {
                const notifTitle = `Order ${status.replace('_', ' ')}`;
                await createNotification(order.store.managerId, ordStoreId, 'ORDER', notifTitle, `Order #${order.orderNumber} status updated to ${status}`, `/orders/${id}`);
            }
        }
        if (order.user?.phone) {
            const smsStatusMap = {
        ...
        await prisma.$transaction(async (tx) => {
            ...
            if (existingMerchant) {
                ...
                await tx.merchant.create({
                    data: { id: userId, ownerName, email, phone, status: 'draft', storeName: null }
                });
            }
        });
    ...
    try {
        ...
        await prisma.$transaction(async (tx) => {
            ...
            if (payload.category !== undefined)
                updateData.verticalId = 'c307b78e-b924-47a1-a5a7-4405777fa50c';
