...
app.post('/orders', async (req, res) => {
    try {
        const {
            ...
            items,
            totalAmount,
            paid,               // NEW: Consumer sends true after successful payment
            paymentId,          // NEW: Razorpay payment ID for audit trail
            orderRequestId,     // NEW: Reference to the original order_request
            customerName,       // NEW: Pickup person name
            customerPhone,      // NEW: Pickup person phone
            storeName,          // NEW: Store display name
            specialInstructions,
            arrivalTime,
            otp                 // NEW: OTP generated by consumer app
        } = req.body;
        ...
        // Logic Gate: If consumer has already paid, mark as CONFIRMED + isPaid
        const orderStatus = paid ? 'CONFIRMED' : 'PENDING';
        ...
        const order = await prisma.$transaction(async (tx) => {
            ...
            // 1. Trigger New Order Notification within tx
