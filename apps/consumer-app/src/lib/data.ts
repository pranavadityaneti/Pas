// @lock — Do NOT overwrite. Approved data as of Feb 27, 2026.
...
export const generateProducts = (category: string, count: number, startId: number) => {
  ...
  const FOOD_SUBS = ["Starters", "Main Course", "Starters", "Main Course", "Desserts", "Beverages", "Main Course", "Starters"];
  ...
  for (let i = 0; i < count; i++) {
    ...
    products.push({
      ...
      subCategory: category === 'food' ? FOOD_SUBS[i % FOOD_SUBS.length] : undefined,
      isVeg: (category === 'food' || category === 'grocery' || category === 'bakery') ? !['Butter Chicken', 'Biryani', 'Chicken Curry Cut', 'Mutton', 'Fish'].some(n => baseName.includes(n)) : undefined
    });
  }
  return products;
};
